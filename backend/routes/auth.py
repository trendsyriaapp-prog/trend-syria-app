...
async def send_registration_otp(request: Request, data: dict):
    """
    إرسال OTP للتسجيل - الخطوة الأولى
    لا يتم إنشاء الحساب حتى يتم التحقق من OTP
    """
    ...
    if existing:
        raise HTTPException(status_code=400, detail="رقم الهاتف مسجل مسبقاً، يرجى تسجيل الدخول")
    
    # إنشاء معرف للتسجيل المعلق
    ...
    try:
        ...
        auth_logger.error(f"Failed to send registration OTP: {e}")
        raise HTTPException(status_code=500, detail="فشل إرسال رمز التحقق")
...
async def verify_registration_otp(request: Request, data: dict, response: Response):
    """
    التحقق من OTP وإنشاء الحساب - الخطوة الأخيرة
    يتم إنشاء الحساب فقط بعد التحقق من OTP
    """
    registration_id = data.get("registration_id")
    otp = data.get("otp", "").strip()
    
    if not registration_id or not otp:
    ...
    expected_otp = TEST_OTP_CODE if TEST_MODE else pending["otp"]
    
    if otp != expected_otp:
        raise HTTPException(status_code=400, detail="رمز التحقق غير صحيح")
    
    # التحقق مرة أخرى من أن الرقم غير مسجل
    existing = await db.users.find_one({"phone": pending["phone"]})
    if existing:
    ...
    phone = pending["phone"]
    full_name = data.get("full_name", pending.get("full_name", ""))
    password = data.get("password", "")
    city = data.get("city", "حلب")
    ...
    initial_role_status = {
        user_type: {
            "status": "active" if user_type == "buyer" else "not_submitted",
            "added_at": datetime.now(timezone.utc).isoformat()
        }
    }
    
    # إعداد البيانات الإضافية حسب نوع المستخدم
    seller_data = data.get("seller_data") if user_type == "seller" else None
    food_seller_data = data.get("food_seller_data") if user_type == "food_seller" else None
    delivery_data = data.get("delivery_data") if user_type == "delivery" else None
    ...
    user_doc = {
        ...
        "is_active": True,
        "wallet_balance": 0,
        "phone_verified": True,  # تم التحقق من الرقم
        "created_at": datetime.now(timezone.utc),
    ...
    if seller_data:
        user_doc["seller_documents"] = {
            ...
            "commercial_registration": seller_data.get("commercial_reg"),
    ...
    if food_seller_data:
        user_doc["food_store"] = {
            "name": food_seller_data.get("store_name"),
            "name_en": food_seller_data.get("store_name_en"),
            "category": food_seller_data.get("business_category"),
            "description": food_seller_data.get("description"),
            "logo": food_seller_data.get("store_logo"),
            "cover_image": food_seller_data.get("store_image"),
            "commercial_license": food_seller_data.get("commercial_license"),
            "city": food_seller_data.get("city", city),
            "area": food_seller_data.get("area"),
            "address_details": food_seller_data.get("address_details"),
            "is_open": False,
            "submitted_at": datetime.now(timezone.utc).isoformat()
        }
        user_doc["role_status"]["food_seller"]["status"] = "pending"
    
    # إضافة بيانات موظف التوصيل
    if delivery_data:
        user_doc["delivery_documents"] = {
            "national_id": delivery_data.get("national_id"),
            "personal_photo": delivery_data.get("personal_photo"),
            "id_photo": delivery_data.get("id_photo"),
            "bike_photo": delivery_data.get("bike_photo"),
            "fuel_type": delivery_data.get("fuel_type"),
            "home_address": delivery_data.get("home_address"),
            "home_latitude": delivery_data.get("home_latitude"),
            "home_longitude": delivery_data.get("home_longitude"),
            "submitted_at": datetime.now(timezone.utc).isoformat()
        }
        user_doc["role_status"]["delivery"]["status"] = "pending"
    ...
    auth_logger.info(f"✅ New user registered with OTP verification: {mask_phone(phone)} ({user_type})")
