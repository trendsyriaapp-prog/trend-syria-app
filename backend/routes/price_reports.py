...
async def get_reports_stats(user: dict = Depends(get_current_user)) -> dict:
    """إحصائيات البلاغات"""
    if user["user_type"] not in ["admin", "sub_admin"]:
        raise HTTPException(status_code=403, detail="غير مصرح")
...
async def suspend_seller(seller_id: str, reason: str = "مخالفة الأسعار", user: dict = Depends(get_current_user)) -> dict:
    """تعليق حساب البائع"""
    if user["user_type"] not in ["admin", "sub_admin"]:
        raise HTTPException(status_code=403, detail="غير مصرح")
...
async def unsuspend_seller(seller_id: str, user: dict = Depends(get_current_user)) -> dict:
