...
@router.post("")
async def create_price_report(report: PriceReportCreate, user: dict = Depends(get_current_user)) -> dict:
...
async def get_my_reports(user: dict = Depends(get_current_user)) -> dict:
...
async def get_reports_stats(user: dict = Depends(require_admin_user)) -> dict:
    """إحصائيات البلاغات"""
...
async def get_seller_violations(user: dict = Depends(get_current_user)) -> dict:
    """جلب مخالفات البائع"""
...
async def suspend_seller(seller_id: str, reason: str = "مخالفة الأسعار", user: dict = Depends(require_admin_user)) -> dict:
    """تعليق حساب البائع"""
    await db.users.update_one(
        {"id": seller_id},
        {"$set": {
...
async def unsuspend_seller(seller_id: str, user: dict = Depends(require_admin_user)) -> dict:
...
async def get_store_price_rating(store_id: str) -> dict:
    """
...
async def get_seller_price_stats(seller_id: str) -> dict:
    """
