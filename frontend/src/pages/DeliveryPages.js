...
const DeliveryPendingApproval = () => {
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      if (status === 'approved') {
        toast({ title: "تم الاعتماد!", description: "تم اعتماد وثائقك. يمكنك الآن استلام الطلبات." });
        navigate('/delivery/dashboard', { replace: true });
      } else if (status === 'rejected') {
        toast({ title: "تم الرفض", description: "تم رفض وثائقك. يرجى إعادة رفعها.", variant: "destructive" });
        navigate('/delivery/documents', { replace: true });
      } else {
